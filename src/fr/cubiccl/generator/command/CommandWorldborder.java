...
public class CommandWorldborder extends Command implements ActionListener
{
	...
	private CEntry entryValue, entryValue2;
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getSource() == this.comboboxMode)
		{
			...
			this.entryValue.label.setTextID("worldborder." + mode);
			...
			if (value2) this.entryValue2.label.setTextID("worldborder." + mode + "2");
		...
		{
			this.entryValue.label.setTextID("worldborder." + this.comboboxMode.getValue() + "." + this.comboboxMode2.getValue());
		}
	}
	...
	public CPanel createGUI()
	{
		...
		++gbc.gridy;
		panel.add((this.entryValue = new CEntry("worldborder.add", "0")).container, gbc);
		++gbc.gridy;
		panel.add((this.entryValue2 = new CEntry("worldborder.add2", "0")).container, gbc);
	...
	public String generate() throws CommandGenerationException
	{
		String mode = this.comboboxMode.getValue(), mode2 = this.comboboxMode2.getValue(), value = this.entryValue.getText(), value2 = this.entryValue2
				.getText(), name = this.entryValue.label.getAbsoluteText(), name2 = this.entryValue2.label.getAbsoluteText();
		...
		switch (mode)
		{
			...
			case "set":
				try
				{
					...
					if (mode.equals("set") && i < 0) throw new WrongValueException(name, Lang.translate("error.integer.positive"), value);
				...
				if (time) try
				{
					...
					if (i < 0) throw new WrongValueException(name2, Lang.translate("error.integer.positive"), value2);
				} catch (NumberFormatException e)
				{
					throw new WrongValueException(name2, Lang.translate("error.integer.positive"), value2);
				}
			...
			case "center":
				...
				{
					throw new WrongValueException(name2, Lang.translate("error.integer"), value2);
				}
			...
			case "warning":
				...
				try
				{
					...
					if (f < 0)
					{
						if (isFloat) throw new WrongValueException(name, Lang.translate("error.number.positive"), value);
						throw new WrongValueException(name2, Lang.translate("error.integer.positive"), value2);
					}
				} catch (NumberFormatException e)
				{
